...
public class InventorySlot : MonoBehaviour, IDropHandler
{
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (transform.childCount == 1)
        {
            ...
            UpgradeRarity(item, currentItem);
        }
    }   
    public bool IsEmpty()
    {
    ...
    public void UpgradeRarity(InventoryItem item, InventoryItem currentItem)
    {
        if (currentItem._weaponData._weaponName == item._weaponData._weaponName && currentItem._weaponData._wpRarity == item._weaponData._wpRarity && 
            (currentItem._weaponData._wpRarity != WeaponRarity.legendary || currentItem._weaponData._wpRarity != WeaponRarity.legendary))
        {
            _inventoryManager.RemoveWeapon(item._inventoryPosition);
